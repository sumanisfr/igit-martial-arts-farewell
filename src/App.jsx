...
function VideoScreen({ visible }) {
  ...
  // Try to use a local video file at `/Seniors.mp4` (place the file in public/)
  const localVideoUrl = "/Seniors.mp4";
  const [hasLocalVideo, setHasLocalVideo] = useState(null);

  // Google Drive folder: convert to embeddable (fallback)
  ...
  // Check if local video exists (HEAD request). If blocked by CORS, this will fail and fallback to Drive.
  useEffect(() => {
    let cancelled = false;
    fetch(localVideoUrl, { method: "HEAD" })
      .then((res) => {
        if (!cancelled) setHasLocalVideo(res.ok);
      })
      .catch(() => {
        if (!cancelled) setHasLocalVideo(false);
      });
...
function SeniorsPage() {
  const [images, setImages] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("seniorImages") || "{}");
    } catch (e) {
      return {};
    }
  });
  ...
  // If some cards don't have images yet, try to auto-detect files in public/avatars
  useEffect(() => {
    let cancelled = false;
    const detect = async () => {
      for (const s of sortedSeniors) {
        if (cancelled) return;
        if (images[s.name]) continue;
        const base = `/avatars/${s.name.toLowerCase()}`;
        try {
          const resPng = await fetch(`${base}.png`, { method: "HEAD" });
          if (resPng.ok) {
            if (cancelled) return;
            setImages((cur) => ({ ...cur, [s.name]: `${base}.png` }));
            continue;
          }
        } catch (e) {}
        try {
          ...
          if (resJpg.ok) {
            if (cancelled) return;
            setImages((cur) => ({ ...cur, [s.name]: `${base}.jpg` }));
            continue;
          }
        } catch (e) {}
      }
  ...
  const handleAddPhoto = (name) => {
    const url = prompt(`Paste a public image URL for ${name} (Instagram/LinkedIn image URL)`);
    if (url) setImages((s) => ({ ...s, [name]: url }));
  };

  const handleRemovePhoto = (name) => {
    if (!confirm(`Remove photo for ${name}?`)) return;
    setImages((s) => { const copy = { ...s }; delete copy[name]; return copy; });
  };
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* make video element better fit small screens when aspect-ratio removed */
        .video-iframe video, .video-iframe iframe {
        ...
        @media (max-width: 900px) {
          .video-container { max-width: 92%; aspect-ratio: 16/9; padding: 8px; }
          .senior-connections { gap: 0.35rem; }
          .connection-link { width: 100%; text-align: center; }
          .controls-bar { max-width: 92%; }
        }
        ...
        @media (max-width: 600px) {
          ...
          /* Mobile: remove strict aspect ratio so the video can size naturally like the screenshot */
          .video-container { aspect-ratio: auto; max-width: 360px; padding: 8px; border-radius: 12px; }
          .video-iframe { height: auto; border-radius: 8px; max-height: 55vh; }
          .video-iframe video, .video-iframe iframe { height: auto; max-height: 50vh; }
          .controls-bar { width: 100%; max-width: 360px; margin-top: 0.6rem; }
          .controls-bar .ctrl-btn, .controls-bar .gold-btn { padding: 0.6rem 0.9rem; font-size: 0.85rem; }
          .play-btn { width: 84px; height: 84px; font-size: 2rem; }
          .overlay-text { font-size: 0.85rem; }
        }
        @media (max-width: 380px) {
          .seniors-grid { grid-template-columns: 1fr; }
          .video-container { max-width: 340px; padding: 6px; }
          .mobile-menu-btn { width: 36px; height: 34px; font-size: 1rem; }
        }
      `}</style>
